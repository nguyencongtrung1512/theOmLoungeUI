...
import { Clock, Minus, Plus, X, ChevronRight, ArrowRight, ChevronLeft, Calendar } from "lucide-react"
...
export function CartModal({ open, onOpenChange, items, onUpdateQuantity, onRemoveItem }: CartModalProps) {
  ...
  const [days, setDays] = React.useState(getDaysInWeek());
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        >
          <SheetHeader className="relative flex items-center justify-center pt-4 pb-2 px-6 border-b border-[#e3d5c1]">
            {step === 'booking' && (
              <button onClick={() => setStep('cart')} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#a08060] hover:text-[#2c1a0e]">
                <ChevronLeft size={20} />
              </button>
            )}
            ...
            {step === 'cart' ? (
                  ...
                  <div key={parent.id}>
                    <div className="flex items-center gap-3 px-5 py-4 border-b border-[#ecdec9]">
                      <div className="w-[50px] h-[50px] overflow-hidden flex-shrink-0 bg-[#e3d5c1]">
                        {parent.image && <img src={parent.image} alt={parent.name} className="w-full h-full object-cover" />}
                      </div>
                      ...
                      <div className="pl-6">
                        {parent.children.map((item) => (
                          <div key={item.id} className="flex items-center gap-3 px-5 py-4 border-b border-[#ecdec9]">
                            ...
                            <button onClick={() => onRemoveItem?.(item.id)} className="text-[#b8a090] hover:text-[#7a5c3e]">
                              <X size={14} />
                            </button>
                          </div>
                        ))}
              ...
              <div className="p-5 flex flex-col gap-6">
                <div className="space-y-4">
                  <div>
                    <label className="text-[0.7rem] text-[#a08060] uppercase font-bold tracking-wider">Tên khách hàng</label>
                    <p className="text-[0.9rem] text-[#2c1a0e] font-semibold border-b border-[#e3d5c1] pb-1">Thuỳ Đỗ</p>
                  </div>
                  <div>
                    <label className="text-[0.7rem] text-[#a08060] uppercase font-bold tracking-wider">Số điện thoại</label>
                    <p className="text-[0.9rem] text-[#2c1a0e] font-semibold border-b border-[#e3d5c1] pb-1">0969-886-969</p>
                  </div>
                </div>

                <div>
                  <label className="text-[0.7rem] text-[#a08060] uppercase font-bold tracking-wider block mb-3">Chọn ngày</label>
                  <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                    {days.map((d, i) => (
                      <button
                        ...
                        className={`flex flex-col items-center justify-center min-w-[80px] py-3 px-2 rounded-sm transition-all flex-shrink-0 ${selectedDate === d.date
                ...
                <div>
                  <label className="text-[0.7rem] text-[#B97951] uppercase font-bold tracking-wider block mb-3">Chọn khung giờ</label>
                  <div className="grid grid-cols-4 gap-2">
                    {TIME_SLOTS.map((time) => (
                      <button
                        key={time}
                        onClick={() => dispatch(setSelectedTime(time))}
                        className={`py-2 border transition-all flex flex-col items-center ${selectedTime === time
          ...
          <div className="border-t border-[#e3d5c1] bg-[#f7f0e6]">
            {step === 'cart' ? (
              <>
                {!showTechnicianList ? (
                  <div className="px-5 py-3 border-b cursor-pointer hover:bg-[#efe6d9] transition-colors" onClick={() => setShowTechnicianList(true)}>
                    <div className="flex justify-between items-center">
                      <span className="text-[0.83rem] text-[#4a3522] font-medium">Kỹ thuật viên</span>
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-2">
                          {technicians[selectedTechnicianIndex]?.avatar && (
                            <img src={technicians[selectedTechnicianIndex].avatar} alt={technicians[selectedTechnicianIndex].name} className="w-6 h-6 rounded-full object-cover" />
                          )}
                          <span className="text-[0.83rem] text-[#2c1a0e] font-medium">{technicians[selectedTechnicianIndex]?.name}</span>
                        </div>
                  ...
                  <div className="border-b border-[#e3d5c1] animate-in fade-in">
                    <div className="px-5 py-3 cursor-pointer hover:bg-[#efe6d9] transition-colors" onClick={() => setShowTechnicianList(false)}>
                      ...
                      <div className="flex flex-col gap-3">
                        {technicians.map((tech, idx) => (
                          <div
                            key={idx}
                            className={`flex items-center gap-2 p-2 rounded cursor-pointer hover:bg-[#efe6d9] transition-colors ${selectedTechnicianIndex === idx ? 'bg-[#e8d5a3] border border-[#c9af93]' : ''
                              }`}
                            onClick={() => {
              ...
              <div className="px-5 py-3">
                <button onClick={handleBooking} className="flex items-center justify-between gap-2 w-full bg-[#824C08] text-[#f7f0e6] py-2 px-5 text-[0.95rem] tracking-wide hover:bg-[#633a06] transition-colors">
